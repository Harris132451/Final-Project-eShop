  ...
  useEffect(() => {
    const generateReviews = async () => {
      ...
      for (let i = 1; i <= 1; i++) {
        ...
        const response = await fetch(`https://dummyjson.com/comments/${random_number}`);
        const data = await response.json();
        const rating = Math.random() >= 0.2 ? 5 : Math.floor(Math.random() * 4) + 1;
      ...
      <article key={index} className="flex items-start mb-8">
        ...
        <div className="flex items-center ml-4">
          {[...Array(5)].map((_, index) => (
            <svg
              key={index}
              className={`w-4 h-4 ${index < review.rating ? 'text-yellow-300' : 'text-gray-300 dark:text-gray-500'}`}
              aria-hidden="true"
