  ...
  return <>
    <section className="py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="font-manrope font-bold text-3xl min-[400px]:text-4xl text-black mb-8 max-lg:text-center">Products List</h2>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {item.map(product => (
              <Link to={`/ProductPage`} className="max-w-[384px] mx-auto" key={product.id}>
              <div className="w-full max-w-sm aspect-square relative overflow-hidden">
                <div className="w-full h-full rounded-xl overflow-hidden hover:scale-105 transition-transform transition-duration-500">
                  <img src={product.picture} alt={product.name} className="w-full h-full rounded-xl" />
                </div>
              </div>
