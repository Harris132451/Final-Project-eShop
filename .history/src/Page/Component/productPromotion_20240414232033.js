...
function ProductPromotion() {
  ...
  return (
    <div className="container px-5 py-24 mx-auto bg-white-200 shadow-lg max-w-4xl truncate inline-block">
    
        {promoContent.slice(0, expanded ? promoContent.length : 5).map((content, index) => (
          <div key={index} className="product-promotion-item my-6 leading-relaxed text-gray-800 truncate transition duration-500 hover:bg-gray-100 hover:text-blue-500 hover:shadow-lg rounded-lg p-4 whitespace-normal">{content}</div>
        ))}
        <div key={index} className="product-promotion-item my-6 leading-relaxed text-gray-800 truncate transition duration-500 hover:bg-gray-100 hover:text-blue-500 hover:shadow-lg rounded-lg p-4 whitespace-normal">
