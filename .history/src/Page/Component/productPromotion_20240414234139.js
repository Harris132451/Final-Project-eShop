...
function ProductPromotion() {
  ...
  const toggleExpandedWithPreventDefault = (event) => {
  ...
  return (
    <div className="container px-10 py-5 mx-auto bg-white shadow-sm rounded-xl max-w-4xl truncate inline-block">
      ...
      {!expanded && 
        <div className={`my-6 leading-relaxed text-gray-800 truncate transition duration-500 ${styleEnabled ? 'hover:bg-gray-100 hover:text-blue-500 hover:shadow-lg' : ''} rounded-lg p-4 whitespace-normal`}>
          <a href="#" className="text-blue-500 hover:underline" onClick={toggleExpandedWithPreventDefault}>Read more</a>
        </div>
      }
    </div>
  );
