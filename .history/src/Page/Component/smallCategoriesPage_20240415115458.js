...
const SmallCategoriesPage = (props) => {
  ...
  const smallcategories = categories.find(
    (cat) => cat.Name === smallCategoriesName
  );
  ...
  const parentCategory = categories.find(
    (cat) => cat.id === smallcategories.Parent
  );
      ...
      <div class="font-[sans-serif]">
        <div class="p-4 mx-auto lg:max-w-6xl max-w-xl md:max-w-full">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {item
              .filter((item) => item.smallcategories === smallcategories.Name)
              .map((filteredItem) => (
                <div
                  key={filteredItem.id}
                ...
                >
                  <button class="bg-white w-10 h-10 flex items-center justify-center rounded-full cursor-pointer absolute top-3 right-3 transition-colors hover:text-blue-500">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18px" class="fill-current text-gray-800 inline-block" viewBox="0 0 64 64">
